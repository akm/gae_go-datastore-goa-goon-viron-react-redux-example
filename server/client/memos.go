...
// ShowMemosPath computes a request path to the show action of memos.
...
// show
func (c *Client) ShowMemos(ctx context.Context, path string) (*http.Response, error) {
	req, err := c.NewShowMemosRequest(ctx, path)
	if err != nil {
		return nil, err
	}
	return c.Client.Do(ctx, req)
...
// NewShowMemosRequest create the request corresponding to the show action endpoint of the memos resource.
func (c *Client) NewShowMemosRequest(ctx context.Context, path string) (*http.Request, error) {
	scheme := c.Scheme
	if scheme == "" {
		scheme = "http"
	}
